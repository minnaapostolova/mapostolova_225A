...
#excluding cells with higher mitochondrial DNA (~5% = good threshold)
#feature = genes/cell (below 300-500 is poor cell quality)
#count = sequences that line up with genome
...
#combine plots into 1 pane by adding "+"
...
#in single cell, cell cycle can be the dominant signature; need to make sure we take this into account
...
#making sure cells in subsequent analysis are of good quality post-filtering


```{r filtered plots}
#use ? mark command in front of function to search documentation of library and ?? to search all of the libraries (even those you haven't loaded) for any relevant documentation
multi_dittoPlot(object = Tcells,
                vars = c("nFeature_RNA", "nCount_RNA","percent.mito"),
                group.by = "orig.ident",
                plots = c("vlnplot","jitter"),
                jitter.width = 1.5,
                jitter.size = 0.25)
#orig.indent above is defined 'SeuratProject'
...
#double-check to make sure mito. genes are not part of your most variable genes (if they are, add a more stringent filter in that code chunk above)
```
...
# all genes all.genes <- c(rownames(Tcells)); takes a very long time
...
Tcells <- RunPCA(
  ...
  verbose = TRUE,
  npcs = 50)
#always calculate more PCs than you need so you can determine where they start to lose statistical significance
```
...
We will use PCheatmaps, PCElbowPlot, and JackStrawPlot to pick which PCs to carry forward into UMAP and clustering.
...
#To pick PCs, look at heatmap to look at which PCs show clear demarcation of cell subsets. Also look at elbow plot to see where the drop off point is.
```
...
dittoDimPlot(Tcells, "Tage", size = 1.5, 
            reduction.use = "umap",
...
#looking at colors in ditto package
dittoColors()
